...
class ArticlesModel extends DefaultModel {
    ...
    public function updateArticle($data) {
        ...
        $query = $db->prepare("UPDATE " . $this->_name . " " .
                                "SET article_type_id = ?, theme_id = ?, title = ?, intro = ?, content = ?, status_id = ?, updated_at = NOW() " .
                                "WHERE id = ?;");
        $query->execute(
            [
                $data['article_type'],
    ...
    public function getArticle($id) {
        $db = $this->connectDb();
        $query = $db->prepare("SELECT a.id, article_type_id, theme_id, author_id, title, intro, content, status_id, created_at, url_id " .
                                ", u.url " .
                                "FROM " . $this->_name . " AS a " .
                                "LEFT JOIN urls AS u " .
                                "ON a.url_id = u.id " .
